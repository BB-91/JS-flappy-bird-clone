...
const _assertIncludesOrNot = (item, arrayOrObject, bool) => {
    ...
    assertNotNull(item);
    if (bool){
        if (!array.includes(item)) {crash(`Item not in array.`, item, array)}
    } else {
        if (array.includes(item)) {crash(`Item already in array.`, item, array)}
    }
}
...
export { TYPE, crash, assertNotNull, assertIncludes, assertDoesNotInclude, assertWithinRange,
