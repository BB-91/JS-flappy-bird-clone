...
imgLoader.addEventListener("error", (error) => {console.log("imgLoader error:", error);})
imgLoader.style.position = "absolute";
imgLoader.style.top = "5000px";
imgLoader.style.display = "none";

/**
 * @param {string} imgSrc - url path to image file
 * @return {Vector2} get the size of the image
 */
const getImageSize = (imgSrc) => {
    try {
        imgLoader.src = imgSrc;
        const size = vector2(imgLoader.naturalWidth, imgLoader.naturalHeight);
        imgLoader.src = "";
        return size;
    } catch (error) {
        console.log(`error in getImageSize():`, error);
    }
    return Vector2.ZERO;
}
...
export { TYPE, crash, assertNotNull, objIncludes, assertIncludes, assertDoesNotInclude, assertHasKey, assertDoesNotHaveKey, assertWithinRange,
                assertTypes, assertInstancesOf, assertDifferentObjects, getNewElement, getPropOrDefault, count, getImageSize };